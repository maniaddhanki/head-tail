...
describe('parseArgs', () => {
  it('should give fileName and default count', () => {
    assert.deepStrictEqual(parseArgs(['a.txt']), { files: ['a.txt'], option: { arg: '-n', key: 'count', limit: 10 } });
  });
  it('should return options and fileName', () => {
    assert.deepStrictEqual(parseArgs(['-n', '5', 'a.txt']), { files: ['a.txt'], option: { arg: '-n', key: 'count', limit: 5 } });
  });
  it('should return byte when -n is specified', () => {
    assert.deepStrictEqual(parseArgs(['-c', '4', 'a.txt']), { files: ['a.txt'], option: { arg: '-c', key: 'byte', limit: 4 } });
  });
  it('should accept multiple files and single option', () => {
    assert.deepStrictEqual(parseArgs(['-n', '5', 'a.txt', 'b.txt']), { files: ['a.txt', 'b.txt'], option: { arg: '-n', key: 'count', limit: 5 } });
  });
  it('should work for options without spaces', () => {
    assert.deepStrictEqual(parseArgs(['-n5', 'a.txt']), { files: ['a.txt'], option: { arg: '-n', key: 'count', limit: 5 } });
  });
...
describe('parseWithOption', () => {
  it('should give count option when -n is specified', () => {
    assert.deepStrictEqual(parseWithOption(['-n', '5', 'a.txt', 'b.txt']), { files: ['a.txt', 'b.txt'], option: { arg: '-n', key: 'count', limit: 5 } });
  });
  it('should give byte option when -c is specified', () => {
    assert.deepStrictEqual(parseWithOption(['-c', '4', 'a.txt', 'b.txt']), { files: ['a.txt', 'b.txt'], option: { arg: '-c', key: 'byte', limit: 4 } });
  });
  it('should overWrite option if repeated', () => {
    assert.deepStrictEqual(parseWithOption(['-n', '5', '-n', '2', 'a.txt', 'b.txt']), { files: ['a.txt', 'b.txt'], option: { arg: '-n', key: 'count', limit: 2 } });
  });
  it('should overWrite option if repeated in other formats also', () => {
    assert.deepStrictEqual(parseWithOption(['-n', '5', '-n2', 'a.txt', 'b.txt']), { files: ['a.txt', 'b.txt'], option: { arg: '-n', key: 'count', limit: 2 } });
  });
  it('should work for options without spaces', () => {
    assert.deepStrictEqual(parseWithOption(['-n5', 'a.txt']), { files: ['a.txt'], option: { arg: '-n', key: 'count', limit: 5 } });
  });
  it('option is count if option does not have key', () => {
...
describe('defaultOption', () => {
  it('should give count:10 as defult option', () => {
    assert.deepStrictEqual(defaultOption(['a.txt', 'b.txt']), { files: ['a.txt', 'b.txt'], option: { arg: '-n', key: 'count', limit: 10 } });
  });
});
...
describe('fetchOptions', () => {
  it('should give all options in arguments', () => {
    const expected = {
      files: ['a.txt', 'b.txt'],
      options: [{ arg: '-n', key: 'count', limit: 5 },
      { arg: '-n', key: 'count', limit: 4 },
      { arg: '-n', key: 'count', limit: 5 }]
    };
...
describe('constructOption', () => {
  it('should contruct option with given values', () => {
    assert.deepStrictEqual(constructOption('-n', '2'), { arg: '-n', key: 'count', limit: 2 });
    assert.deepStrictEqual(constructOption('-c', '5'), { arg: '-c', key: 'byte', limit: 5 });
  });
});
