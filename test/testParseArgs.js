...
describe('parseArgs', () => {
  ...
  it('should give byte option when -c is specified', () => {
    assert.deepStrictEqual(parseArgs(['-c', '4', 'a.txt', 'b.txt']), { files: ['a.txt', 'b.txt'], option: { flag: '-c', countBy: 'byte', value: 4 } });
  });
  it('should overWrite option if repeated', () => {
    assert.deepStrictEqual(parseArgs(['-n', '5', '-n', '2', 'a.txt', 'b.txt']), { files: ['a.txt', 'b.txt'], option: { flag: '-n', countBy: 'line', value: 2 } });
  });
  it('should overWrite option if repeated in other formats also', () => {
    assert.deepStrictEqual(parseArgs(['-n', '5', '-n2', 'a.txt', 'b.txt']), { files: ['a.txt', 'b.txt'], option: { flag: '-n', countBy: 'line', value: 2 } });
  });
...
describe('defaultOption', () => {
  it('should give count:10 as defult option', () => {
    assert.deepStrictEqual(defaultOption(['a.txt', 'b.txt']), { flag: '-n', countBy: 'line', value: 10 });
  });
});
...
describe('fetchOptions', () => {
  ...
  it('should give empty array when no options are specified', () => {
    const expected = {
      files: ['a.txt', 'b.txt'],
      options: [{ flag: '-n', countBy: 'line', value: 10 }]
    };
