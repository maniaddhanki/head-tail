...
describe('headFile', () => {
  it('should give object of file name,headed contents and error status', () => {
    const readFunction = mockReadFileSync(['a.txt'], 'utf8', ['a\nb\nc']);
    assert.deepStrictEqual(headFile(readFunction, 'a.txt', { flag: '-n', limit: 2 }), { file: 'a.txt', output: 'a\nb', isError: false });
  });
  it('isError should be true if head was not successfull', () => {
    const readFunction = mockReadFileSync(['b.txt'], 'utf8', ['a\nb\nc']);
    assert.deepStrictEqual(headFile(readFunction, 'a.txt', { flag: '-n', limit: 2 }), { file: 'a.txt', output: 'head: a.txt: No such file or directory', isError: true });
  });
});
